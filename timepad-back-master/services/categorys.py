...
def create_category(data: CategoryDto ,db: Session):
    category = Categorys(name=data.name, description=data.description,img=data.img,date=data.date,location=data.location,status=data.status,members=data.members)
    try:
        db.add(category)
        db.commit()
        db.refresh(category)
